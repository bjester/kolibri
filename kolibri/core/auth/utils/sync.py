...
def merge_filter(filter_a, filter_b):
    """
    Should Morango's Filter be tolerant of combining with None? Could be symptomatic of an issue if
    unexpected.
    """
    if filter_a is None:
        return filter_b
...
class ClassroomPartitionFilterFactory(ClassroomPartitionFactory):
    ...
    def __init__(self, dataset_id):
        super(ClassroomPartitionFilterFactory, self).__init__(dataset_id)
    ...
    def build(self, collection_id, filter_suffix=None):
        ...
        return super(ClassroomPartitionFilterFactory, self).build(
            collection_id,
    ...
    def build_for_user(self, user_id):
        ...
        for suffix, collection_ids in collection_groups.items():
            for collection_id in collection_ids:
                ...
                combo_filter = merge_filter(combo_filter, collection_filter)
